...
helpers::YamlParser::YamlParser(const std::string path) {
...
auto helpers::YamlParser::nextLine() -> std::string {
    std::string temp = "";  
    int count = position;   
    while(data.at(count) != '\n') {
		if (data.at(count) == ' ') {
...
auto helpers::YamlParser::nextStringString() -> std::pair<std::string, std::string> {
...
auto helpers::YamlParser::nextStringInt() -> std::pair<std::string, int> {
    std::string buffer = nextLine(); // Get the next line.
    int position = buffer.find(":");    // Find where the tag ends
